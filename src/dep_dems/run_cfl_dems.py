...
from src.dep_dems.cfl_config import *
...
def main(exp_id=-1, ref_exp_id=0, plot_order_nd=None, plot_order_yd=None):
    ...
    # load data
    dataset = 'cohort2'
    X, X_test, Y, Y_test, _, _ = load_data(dataset, dems=True)

    results_path = os.path.join(RESULTS_PATH, 'dep_dems/cfl_results')
    ...
    rp_nodem = os.path.join(RESULTS_PATH, 'dep_cfl/cfl_results')
    xlbls_test_nodems = exp_load(rp_nodem, ref_exp_id, 'test', 'CauseClusterer',
                                 'x_lbls')
    
    plot_sankey([xlbls_test_nodems, xlbls_test], os.path.join(fig_path, 'sankey_test.png'))
    ...
    # reload lesion data without dems for vis
    _, X_test,_,_,_,_ = load_data(dataset, dems=False)
    # plot lesions
    fig,ax = plt.subplots(n_clusters,1,figsize=(PW//2,1*n_clusters))
    vmax = np.max([np.mean(X_test[xlbls_test==plot_order_yd[i]],axis=0) for i in range(n_clusters)])
    ...
    fig,ax = plt.subplots(n_clusters,1,figsize=(PW,0.9*n_clusters))
    vmax = np.max([np.mean(X_test[xlbls_test==plot_order_yd[i]],axis=0) for i in range(n_clusters)])
    for i in range(n_clusters):
        masks = X_test[xlbls_test==plot_order_yd[i]]
        orthoslicer = lesion_heatmap(masks, mode='mean', ax=ax[i], 
                                    cut_coords=None, vmax=vmax,
    ...
    for lbls,fn,plot_order in zip([xlbls_test_nodems, xlbls_test],['nodem','dem'],
        ...
        for i in range(n_clusters):
            for j in range(dems_test.shape[1]):
                ...
                # ax[i,j].hist(dems_test[:,j], color='black', histtype='step')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--plot_order_nd', type=int, nargs='+', default=None)
    parser.add_argument('--plot_order_yd', type=int, nargs='+', default=None)
