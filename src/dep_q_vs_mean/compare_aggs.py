...
def compute_cfl_aggs(Y):
    ...
    Y_cfl = np.zeros((Y.shape[0],len(np.unique(CFLC))))
    for i in range(len(np.unique(CFLC))):
        Y_cfl[:,i] = np.mean(Y[:,CFLC==i], axis=1)
    return Y_cfl
...
def plot_aggs(Y_mbdi, Y_sc, Y_cfl, labels):
    ...
    for i,c in enumerate([labels+1, Y_mbdi]):
        ...
        cb_friendly_cmap = LinearSegmentedColormap.from_list('cb_friendly_cmap', 
                                        ['#E0A46C', '#B5CCFF', '#C49FE0', '#B12FE0'], N=4)
        cmap = cb_friendly_cmap if i==0 else 'viridis'
...
def evalute_predictive_ability(Y_mbdi, Y_sc, Y_cfl, Y, labels):
    ''' predict CFL effect macrostate from each agg config '''

    aggs = [Y_mbdi, Y_sc, Y_cfl, Y]
    ...
    for i in range(len(aggs)-1):
        t,p = ttest_ind(scores[i], scores[-1], alternative='less')
        print(f'{t:.2f}, {p:.2f}')

    print('mbdi vs cflaggs', ttest_ind(scores[0],scores[2], alternative='less'))


    # plot scores
    fig,ax = plt.subplots(figsize=(2,2))
    ax.bar(range(len(aggs)), np.mean(scores,axis=1), yerr=sem(scores,axis=1)) 
    ax.set_xticks(range(len(aggs))) 
    ax.set_xticklabels(['mean BDI', 'som-cog', 'cfl aggregates', '21q'], rotation=45, ha='right')    
...
def main(exp_id=0):
    ...
    with open(f'results/dep_cfl/cfl_results/experiment{exp_id_str}/test/EffectClusterer_results.pickle', 'rb') as f:
    ...
    evalute_predictive_ability(Y_mbdi, Y_sc, Y_cfl, Ytest, labels)
