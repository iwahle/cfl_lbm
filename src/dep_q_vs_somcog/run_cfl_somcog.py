...
from src.dep_q_vs_somcog.cfl_config import *
from cfl.util.experiment_loading import exp_load
plt.rcParams["font.size"] = FS-1

fig_path = os.path.join(FIG_PATH, 'dep_q_vs_somcog')
if not os.path.exists(fig_path):
    os.makedirs(fig_path)

def main(exp_id=-1, ref_exp_id=0, plot_order_sc=None, plot_order_q=None):
    ...
    Ysc = np.zeros((Yraw.shape[0], len(np.unique(SOMCOG))))
    for i in range(len(np.unique(SOMCOG))):
        Ysc[:,i] = np.mean(Yraw[:,SOMCOG==i], axis=1)
    Y,Y_test = Ysc[train_idx], Ysc[test_idx]
    Ymean,Ystd = np.mean(Y),np.std(Y)
    Y = (Y-Ymean)/Ystd
    Y_test = (Y_test-Ymean)/Ystd
    print(Y_test.shape)

    results_path = os.path.join(RESULTS_PATH, 'dep_q_vs_somcog/cfl_results')
    ...
    xlbls_test_q = exp_load(rp_q, ref_exp_id, 'test', 'CauseClusterer',
                                 'x_lbls')
    
    plot_sankey([xlbls_test_q, xlbls_test], os.path.join(fig_path, 'sankey.png'))

    n_clusters = len(np.unique(xlbls_test))
    if plot_order_sc is None:
        plot_order_sc = range(n_clusters)
    else:
        assert len(plot_order_sc)==n_clusters
    ...
    cmat = cmat[plot_order_sc][:,plot_order_q]
    ...
    ax.set_yticklabels(['C1\'', 'C2\'', 'C3\'']) #[plot_order_sc])
    ax.set_xlabel('21 questions')
    ax.set_ylabel('som-cog.aff.')
    ...
    vmax = np.max([np.mean(X_test[xlbls_test==plot_order_sc[i]],axis=0) for i in range(n_clusters)])
    for i in range(n_clusters):
        masks = X_test[xlbls_test==plot_order_sc[i]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--plot_order_sc', type=int, nargs='+', default=None)
    parser.add_argument('--plot_order_q', type=int, nargs='+', default=None)

    main(**vars(parser.parse_args()))

    # python src/dep_q_vs_somcog/run_cfl_somcog.py --exp_id 0 --ref_exp_id 0 --plot_order_sc 1 0 2